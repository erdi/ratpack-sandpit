...
class HandlersSpec extends Specification {
    ...
    def "10 - can log a warning when there is a request for a user starting with 'f'"() {
        given:
            def logger = TestLoggerFactory.getTestLogger(UserHandlers)

        when:
            get("user/$username/tweets")

        then:
            logger.allLoggingEvents.contains LoggingEvent.info("Warning, request for $username")
    ...
    def "11 - can log all requests"() {
        given:
            def logger = TestLoggerFactory.getTestLogger("Ratpack")

        when:
            get(requestPath)

        then:
            logger.allLoggingEvents*.message.first().contains(expectedLogEntry)
    ...
    def "13 - only recognized soap actions are handled"() {
        given:
            requestSpec { RequestSpec req ->
                req.headers.set("SOAPAction", "getMessages")
            }

        expect:
            post("api/ws").statusCode == NOT_FOUND.code()
    }
}
